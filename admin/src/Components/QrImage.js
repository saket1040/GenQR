...
import { Container, Card, CardContent, Grid, makeStyles, Button } from '@material-ui/core';
...
function QrImage() {
    ...
    const generateQrcode = async (e) => {
        ...
        axios.post("http://localhost:5000/api/scanQrcode", { url: respoJSON.url }).then(response => {
    ...
    return (
        <div>
            <Container className={classes.container}>
                    ...
                    <CardContent>
                        <Grid container spacing={2}>
                            <Grid item xl={4} lg={4} md={6} sm={12} xs={12}>
                                ...
                                <Button variant="contained" color="primary" className={classes.btn} onClick={generateQrcode}>Generate-QRcode</Button>
                                <br />
                            </Grid>
                            <Grid item xl={4} lg={4} md={6} sm={12} xs={12}>
                                {
                                    url.length > 0 && qrImage
                                        ?
                                        <>
                                            <a href={qrImage} download><img src={qrImage} alt="" /></a>
                ...
                <NavLink to='/'>
                    <Button variant="outlined" color="primary" className={classes.btn} >Link QR Code</Button>
                </NavLink>
            </Container>
        </div>
